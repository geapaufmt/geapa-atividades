...
`ID_PESSOA` e a chave tecnica principal para novos vinculos individuais em atividades, apresentacoes, chamadas, justificativas, certificados e participacoes.
...
`atividadesV2_complementarIdPessoaTodasAbasDev()` e a funcao manual recomendada para uso normal. Ela percorre as abas `Atividades_Apresentacoes`, `Atividades_Presencas_Registros`, `Justificativas_Faltas`, `PORTAL_APRESENTACOES`, `PORTAL_FREQUENCIA_MEMBROS` e `PORTAL_JUSTIFICATIVAS`, preservando IDs ja preenchidos.
